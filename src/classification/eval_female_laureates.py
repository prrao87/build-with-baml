...
def print_category_table(
    ...
    for category in categories:
        ...
        # Get model counts
        genderapi_count = datasets["GenderAPI"].filter(
            (pl.col("category") == category) & (pl.col("gender") == "female") & (pl.col("type") == "laureate")
        ).shape[0]
        
        llm1_count = datasets["GPT-4o-mini"].filter(
            (pl.col("category") == category) & (pl.col("gender") == "female") & (pl.col("type") == "laureate")
        ).shape[0]
        
        llm2_count = datasets["Gemma3-12b"].filter(
            (pl.col("category") == category) & (pl.col("gender") == "female") & (pl.col("type") == "laureate")
        ).shape[0]
        
        llm3_count = datasets["Gemma3-27b"].filter(
            (pl.col("category") == category) & (pl.col("gender") == "female") & (pl.col("type") == "laureate")
        ).shape[0]
...
def print_accuracy_table(
    ...
    print(f"{'Model':<15} {'Count/Reference':<15}")
    print("-" * 35)
    
    for result in results:
        print(
            f"{result['Model']:<15} "
            f"{result['Count']}/{result['Reference']}"
        )
...
if __name__ == "__main__":
    compare_gender_results(
        reference_path="../../data/nobel_laureates/hand_annotated/female_laureates_until_2022.csv",
        genderapi_path="../../data/nobel_laureates/with_gender/scholars_from_genderapi.jsonl",
        llm1_path="../../data/nobel_laureates/with_gender/scholars_from_baml_gpt-4o-mini.jsonl",
        llm2_path="../../data/nobel_laureates/with_gender/scholars_from_baml_gemma3_12b.jsonl",
        llm3_path="../../data/nobel_laureates/with_gender/scholars_from_baml_gemma3_27b.jsonl",
    )
