...
def load_datasets(
    human_annotated_path: str = "../../data/nobel_laureates/human_annotated/scholars_random.csv",
...
def print_mismatches(results: List[Dict]) -> None:
    ...
    for result in results:
        if result["Mismatches"] is not None and result["Mismatches"].height > 0:
            print(f"\n{result['Model']} - Incorrect Predictions:")
...
def main(show_mismatches: bool = False, show_only_mismatches: bool = False):
    ...
    # Print summary table
    print("\nAccuracy comparison for female genders across all results:")
    print_table(results)
